...
export function generateStaticParams() {
  return useCases.filter((item) => item.category === "business").map((item) => ({ slug: item.slug }));
}
...
export default async function BusinessUseCasePage({ params }: { params: Promise<{ slug: string }> }) {
  const useCase = getUseCase("business", (await params).slug);
  if (!useCase) notFound();
