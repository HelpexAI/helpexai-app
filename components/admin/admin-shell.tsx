...
import { Activity, BarChart3, BookOpen, BriefcaseBusiness, CreditCard, FileText, HeartPulse, LayoutDashboard, MessageSquare, Settings, Users } from "lucide-react";
...
export function AdminShell({ email, role, children }: { email: string; role: string; children: React.ReactNode }) {
  ...
  return <div className="min-h-screen bg-zinc-50 text-zinc-950 dark:bg-zinc-950 dark:text-white">
    <aside className="fixed inset-y-0 left-0 hidden w-64 flex-col border-r border-zinc-800 bg-[#07150f] text-white lg:flex">
      <div className="border-b border-white/10 p-6"><Link href="/admin" className="flex items-center gap-3 font-bold"><span className="flex size-9 items-center justify-center rounded-lg bg-emerald-500"><Activity className="size-4" /></span>HelpexAI Admin</Link><p className="mt-3 truncate text-xs text-white/50">{email} · {role}</p></div>
