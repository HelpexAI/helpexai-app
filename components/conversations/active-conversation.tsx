...
function Sources({ sources }: { sources: MessageSource[] }) {
  ...
  return (
    <div className="overflow-hidden rounded-lg border border-zinc-200 bg-zinc-50 dark:border-zinc-700 dark:bg-zinc-800/60">
      ...
      {open && <div className="space-y-2 border-t border-zinc-200 p-3 dark:border-zinc-700">{sources.map((source, index) => <div key={`${source.docId}-${index}`} className="block w-full rounded-lg border-l-2 border-theme-primary bg-white p-3 text-left dark:bg-zinc-900"><span className="flex items-center justify-between gap-2 text-xs font-semibold"><span className="flex min-w-0 items-center gap-1.5"><FileText className="size-3 shrink-0 text-theme-primary" /><span className="truncate">{source.docName}</span><span className="shrink-0 text-zinc-500">{source.pageNumber ? `Page ${source.pageNumber}` : "Find page"}</span></span></span><span className="mt-1 block whitespace-pre-wrap text-xs italic leading-5 text-zinc-500">{source.excerpt}</span></div>)}</div>}
    </div>
  );
}
