...
export function CitationPreviewPanel({ source, onClose }: { source: MessageSource; onClose: () => void }) {
  ...
  const isReport = source.sourceType === "report";
    ...
    <>
      ...
      <aside className="fixed inset-y-0 right-0 z-50 flex w-full max-w-md flex-col border-l border-zinc-200 bg-white shadow-2xl dark:border-zinc-800 dark:bg-zinc-900 lg:static lg:z-auto lg:w-[420px] lg:shrink-0 lg:shadow-none">
        <header className="flex min-h-16 items-center justify-between gap-3 border-b border-zinc-200 px-4 dark:border-zinc-800">
          <div className="min-w-0"><p className="truncate text-sm font-bold">{source.itemTitle ?? source.docName}</p><p className="text-xs text-zinc-500">{isReport ? "Referenced finalized report" : page ? `Page ${page}${preview?.pageCount ? ` of ${preview.pageCount}` : ""}` : "Referenced document text"}</p></div>
        ...
        <div className="public-tool-scrollbar flex-1 space-y-4 overflow-y-auto p-4">
          {!isReport && !preview && !error && <div className="flex min-h-64 items-center justify-center gap-2 text-sm text-zinc-500"><Loader2 className="size-4 animate-spin text-theme-primary" /> Loading cited source...</div>}
          {isReport && <section className="rounded-xl border border-theme-border bg-theme-soft p-4 dark:bg-theme-soft-dark"><p className="mb-2 text-xs font-bold uppercase tracking-wide text-theme-primary">Referenced report content</p><p className="whitespace-pre-wrap text-sm leading-6 text-zinc-700 dark:text-zinc-200">{source.excerpt}</p></section>}
        ...
        <footer className="border-t border-zinc-200 p-4 dark:border-zinc-800"><Link href={isReport ? `/reports/${source.docId}` : `/documents/${source.docId}?${fullDocumentQuery}`} className="flex h-11 items-center justify-center gap-2 rounded-xl bg-theme-primary text-sm font-semibold text-white"><ExternalLink className="size-4" /> {isReport ? "Open full report" : `Open full document${page ? ` on page ${page}` : ""}`}</Link></footer>
      </aside>
    </>
  );
}
