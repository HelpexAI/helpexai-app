...
export function ArticlePage({ article }: { article: SeoArticle }) {
  const productHref = `/${article.category}`;
  const jsonLd = {
          ...
          <main className="mx-auto w-full max-w-4xl flex-1 pt-10 sm:pt-14">
            ...
            <section className="mt-7 rounded-3xl bg-[#0a1628] p-7 text-white sm:p-9">
              ...
              <div className="mt-6 flex flex-col gap-3 sm:flex-row">
                ...
                <Link href={productHref} className="rounded-full border border-white/20 px-6 py-3 text-center text-sm font-semibold">View {article.category === "legal" ? "Helpex Legal" : "Helpex Business"}</Link>
              </div>
            </section>
          </main>
          <MarketingFooter />
