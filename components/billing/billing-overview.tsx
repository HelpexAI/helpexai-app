    ...
    <div className="flex flex-col gap-6 p-4 sm:p-6 lg:p-8">
      ...
      {subscriptionStatus === "past_due" && (
        <div className="rounded-xl border border-red-200 bg-red-50 px-4 py-3 text-sm font-medium text-red-700 dark:border-red-900 dark:bg-red-950/40 dark:text-red-300">
          Your Stripe payment is past due. Update your payment method to restore Pro.
        </div>
      )}
      ...
      <section className="grid gap-5 lg:grid-cols-2">
        ...
        <article className="flex flex-col gap-5 rounded-2xl bg-theme-primary p-6 text-white shadow-lg">
          ...
          <ul className="flex flex-1 flex-col gap-2 border-t border-white/20 pt-5 text-white [&_svg]:text-white">
            <PlanFeature enabled>50 documents</PlanFeature><PlanFeature enabled>50 questions/day</PlanFeature><PlanFeature enabled>Unlimited conversations</PlanFeature><PlanFeature enabled>Advanced citations</PlanFeature><PlanFeature enabled>Priority processing</PlanFeature><PlanFeature enabled>Cancel anytime</PlanFeature>
          </ul>
          {plan === "pro" ? (
            ...
            <button onClick={() => void openStripe("/api/stripe/checkout", "checkout")} disabled={loading !== null} className="flex h-11 items-center justify-center gap-2 rounded-full bg-white font-semibold text-theme-primary disabled:opacity-70">
